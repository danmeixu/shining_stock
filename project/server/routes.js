...
function getRecStock(req, res) {
  ...
  var query = `
    ...
    From data_2018 f1 JOIN Company c ON c.ticker = f1.ticker JOIN daily_Prices d ON d.ticker = c.ticker
    WHERE c.sector = (SELECT sector FROM Company C2 WHERE C2.ticker = '${inputStock}') 
      AND f1.Gross_Profit >= (
        SELECT AVG(Gross_Profit) 
        FROM data_2018 F2 JOIN Company c3 ON c3.ticker = F2.ticker
        WHERE c3.sector = c.sector) 
      AND f1.Gross_Profit >= (SELECT Gross_Profit FROM 2017_Financial_Data F3 WHERE F3.ticker = '${inputStock}') 
      AND f1.Cost_Revenue <= (
        SELECT AVG(Cost_Revenue)
        FROM data_2018 F4 JOIN Company c4 ON c4.ticker = F4.ticker
